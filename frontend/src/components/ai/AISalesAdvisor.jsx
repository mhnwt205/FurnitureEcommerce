...
export default function AISalesAdvisor({ open = false, onOpenChange, launcherRef }) {
  ...
  const performRequest = async ({ text, clientMessageId = createClientMessageId(), consumeMessageId = null, reset = false, retry = false } = {}) => {
    ...
    try {
      const response = await aiAdvisorService.chatWithAdvisor({ message: text, sessionId: state.sessionId, clientMessageId, context: currentProductId ? { currentProductId } : {}, ...(reset ? { resetSession: true } : {}) }, { signal: controller.signal });
      const normalized = normalizeAdvisorResponse(response);
      if (reset) { dispatch({ type: 'RESET_SUCCESS', sessionId: normalized.sessionId, expiresAt: normalized.expiresAt, greeting: greeting() }); return; }
