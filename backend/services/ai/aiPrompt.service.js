...
const buildPrompt = (message, metadata, candidates, conversation) => {
  ...
  return [
    ...
    '## OUTPUT SCHEMA', `Return one JSON object only, with exactly the top-level keys answer and recommendations. answer is a trimmed non-empty string; answer <= ${AI_ANSWER_MAX_LENGTH} characters. recommendations is an array of at most ${AI_MAX_RECOMMENDATIONS} items; recommendations may be []; each item has exactly id and reason, with a positive integer unique allow-listed id [${allowedCandidateIds.join(', ')}] and trimmed non-empty reason; reason <= ${AI_REASON_MAX_LENGTH} characters. Do not return a Product DTO, price, stock, image, category, finalPrice, provider metadata, memoryPatch, or any other field. Return no Markdown or code fence.`,
