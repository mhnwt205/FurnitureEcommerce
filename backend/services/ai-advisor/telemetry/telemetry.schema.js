...
export const aiTelemetryEventNames = [
  ...
  'ai_candidate_pipeline_completed', 'ai_idempotency_cache_hit'
];
...
export const aiTelemetryMetadataSchema = z.object({
  ...
  fallbackUsed: z.boolean().optional(), diversificationApplied: z.boolean().optional(), writerUsed: z.boolean().optional(), writerFallbackUsed: z.boolean().optional(), groundedReasonFallbackCount: z.number().int().min(0).max(5).optional(), sessionAction: z.enum(['created', 'reused', 'reset', 'expired', 'rotated', 'capacity']).optional()
}).strict();
