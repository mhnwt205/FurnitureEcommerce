...
const makeDependencies = (overrides = {}) => {
  const calls = { parse: 0, config: 0, retrieval: 0, prompt: 0, provider: 0, rebuild: 0, fallback: 0, noResult: 0 };
  return {
    calls,
    dependencies: {
...
test('orchestrates parse/config/retrieval/prompt/provider/rebuild exactly once for provider success', async () => {
  ...
  assert.deepEqual(calls, { parse: 1, config: 1, retrieval: 1, prompt: 1, provider: 1, rebuild: 1, fallback: 0, noResult: 0 });
  assert.deepEqual(Object.keys(result.response), ['answer', 'recommendations']);
  assert.deepEqual(result.internal, { providerFallbackUsed: false, providerFailureCode: null, source: 'provider' });
});
...
test('uses backend fallback exactly once for provider failure without refetching', async () => {
  ...
  const { calls, dependencies } = makeDependencies({ callAiProvider: async () => {
    providerCalls += 1;
    return { ok: false, error: { code: 'AI_PROVIDER_TIMEOUT' }, provider: { attemptCount: 2, fallbackUsed: true } };
  } });
  const result = await processAiChat({ message: 'Tìm sofa' }, dependencies);
  assert.deepEqual(calls, { parse: 1, config: 1, retrieval: 1, prompt: 1, provider: 0, rebuild: 0, fallback: 1, noResult: 0 });
  assert.equal(providerCalls, 1);
  assert.deepEqual(result.internal, { providerFallbackUsed: true, providerFailureCode: 'AI_PROVIDER_TIMEOUT', source: 'fallback' });
});
...
test('returns deterministic no-result without prompt or provider when retrieval is empty and propagates retrieval errors', async () => {
  ...
  assert.deepEqual(empty.calls, { parse: 1, config: 1, retrieval: 1, prompt: 0, provider: 0, rebuild: 0, fallback: 0, noResult: 1 });
...
test('short-circuits provider, fallback, and no-result when the prompt builder throws an internal prompt error', async () => {
  ...
  assert.deepEqual(calls, { parse: 1, config: 1, retrieval: 1, prompt: 1, provider: 0, rebuild: 0, fallback: 0, noResult: 0 });
